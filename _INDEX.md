...
## Structural Refs — Call-graph outputs (read by /integrity pre-step)

> Generated by `graphify --no-viz` · Updated manually when scripts/agents change significantly
> Read **God Nodes + Surprising Connections sections only** for integrity checks

| File | What it covers |
|---|---|
| `D:\aidirectory\scripts\graphify-out\GRAPH_REPORT.md` | scripts/ call graph — 129 nodes, 152 edges; `WorkflowExtractor` (22 edges) is highest blast-radius node |
| `D:\aidirectory\Projects\SHAW\agents\graphify-out\GRAPH_REPORT.md` | agents/ structural graph — 74 nodes, 70 edges; `check_n8n()` (6 edges) is n8n API bottleneck; infra-ops→docker-ops→n8n chain confirmed (agents moved to Projects\SHAW\ 2026-07-13) |
...
| [[audits/REVIEW_2026-06-10_graphify-trial]] | Graphify trial — scripts/ graph output + Phase 3 gate evaluation (PASS) |
...
> **Pipeline (2026-07-14):** written LIVE by the `SessionEnd` hook `scripts/memsearch-extract.ps1`
> — transcript → OpenRouter/deepseek summarize → `EXTRACT_<date>_<slug>.md` (YAML content) → indexed
> into memsearch (local ONNX bge-m3 embeddings + Milvus Lite). Search: `memsearch search "<query>"`.
> `mine_patterns.py` reads these (+ legacy `.yml`) to propose CLAUDE.md rules.
> Replaced the old n8n + LLMLingua + Qdrant chain — see `audits/DECISION_pipeline-architecture.md`.
> (Sample legacy extracts below; the live corpus is the full `extracts\` folder, not curated here.)
...
Search past session memory                   → memsearch search "<query>" (WSL2, local, no API key)
```

> **Session-memory pipeline (2026-07-14):** capture is automatic via the `SessionEnd` hook
> (`scripts/memsearch-extract.ps1`) → memsearch (ONNX bge-m3 + Milvus Lite). The old
> n8n/LLMLingua/Qdrant chain is retired for this purpose (containers stopped, kept for possible
> reuse). See `audits/DECISION_pipeline-architecture.md`.
